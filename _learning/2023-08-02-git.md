...
```
gd main..branchname --shortstat
```
# sharing git changes when everything is down
```sh
# if ya want to share some git code with friends
git diff > lala.patch
# share the patchfile with friend (you have to have same base), and they do:
git apply lala.patch
```
# Git Bundle
```sh
