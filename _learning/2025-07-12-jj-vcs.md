...
```sh
# split file from commit
jj diff -s

# split specified path before the current commit
jj split FILE_PATH

# like git checkout back to original
jj restore FILEPATH

# abandon all local jj commits that are not on TRUNK
jj log -r 'mutable()'
jj abandon 'mutable()'

jj bookmark move --from @-
```
---
