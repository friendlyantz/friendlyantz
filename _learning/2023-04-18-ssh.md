...
in `/etc/ssh/sshd_config` update(or do it here `vi /etc/ssh/ssh_config.d/*.conf` if on Ubuntu/Debian amd you want to keep default config allowing passwords - will be applied to other account [refer](https://unix.stackexchange.com/questions/727492/passwordauthentication-no-but-i-can-still-login-by-password)):
